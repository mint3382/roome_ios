...
class SharingCardView: UIView {
    private let gradientLayer = CAGradientLayer()
    private let sharingContainer: SharingContainer
    
    init(frame: CGRect, sharingContainer: SharingContainer) {
    ...
    private func configureBackgroundColor() {
        ...
        guard let color = sharingContainer.profile?.data.color else {
            return
        }
    ...
    private func configureIntroduceStackView() {
        ...
        let roomCount = ProfileLabel(text: sharingContainer.profile?.data.count, isIntroduceLine: true)
    ...
    func configureMBTILable() {
        guard sharingContainer.profile?.data.mbti != "NONE" else {
            return
        }
        
        let mbtiLabel = ProfileLabel(text: sharingContainer.profile?.data.mbti, isIntroduceLine: true)
    ...
    private func configureLineStackView1() {
        let labels = sharingContainer.profile?.data.preferredGenres.compactMap { ProfileLabel(text: $0.title) }
    ...
    private func configureLineStackView2() {
        ...
        let labels = sharingContainer.profile?.data.userStrengths.compactMap { ProfileLabel(text: $0.title) }
    ...
    private func configureLineStackView3() {
        let labels = sharingContainer.profile?.data.themeImportantFactors.compactMap { ProfileLabel(text: $0.title) }
    ...
    private func configureLineStackView4() {
        let positionLabel = (sharingContainer.profile?.data.horrorThemePosition
            .map { ProfileLabel(text: $0.title) }!)!
        let hintLabel = (sharingContainer.profile?.data.hintUsagePreference
            .map { ProfileLabel(text: $0.title) }!)!
    ...
    private func configureLineStackView5() {
        let lockLabel = (sharingContainer.profile?.data.deviceLockPreference
            .map { ProfileLabel(text: $0.title) }!)!
        let activityLabel = (sharingContainer.profile?.data.activity
            .map { ProfileLabel(text: $0.title) }!)!
    ...
    private func configureLineStackView6() {
        let labels = sharingContainer.profile?.data.themeDislikedFactors.compactMap { ProfileLabel(text: $0.title) }
