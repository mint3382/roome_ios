...
class ActivityViewModel {
    ...
    private func checkEdit() {
        let userSelect =  UserContainer.shared.defaultProfile?.data.activities.filter {
            $0.id == id
        }[0].id
        let profileItem = UserContainer.shared.profile?.data.activity.map { $0.id }
    ...
    func handlePage(id: Int) {
        Task {
            do {
                try await useCase.activityWithAPI(id: id)
